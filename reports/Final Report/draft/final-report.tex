...
\section{Oplossingen / Highlights}
	...
	Ieder monument heeft een beschrijving. De lengte van de beschrijving verschilt per monument en is veelal tussen de 5 en 200 woorden. Uit een dergelijk grote dataset van $\pm$ 25000 monumenten kan interessante data worden herleid.
		...
		\begin{equation}
			IDF = \log{\frac{\text{total}}{\text{subset}}}
		\end{equation}
		...
		\begin{equation}
			\text{Score woord} = \frac{\text{occ}}{\text{totalocc}} \cdot \log{\frac{\text{total}}{\text{subset}}}
		\end{equation}
	...
	\begin{equation}
		\text{Score woord i category j} = p_{ij} = \frac{occ_{ij}}{occ_{i\*}} \cdot \log{\frac{count(j)}{occ_{i\*}}}
	\end{equation}
	
	Wanneer een ongecategoriseerd monument gecategoriseerd moet worden, extraheert CultuurApp zoals beschreven sleutelwoorden uit de tekst. Voor ieder van deze sleutelwoorden, die de belangrijkste woorden zijn van de tekst, wordt per categorie berekend wat de waarschijnlijkheid is dat een monument tot een categorie behoort. Dit gebeurt door de TF-IDF waarde van het sleutelwoord te vermenigvuldigen met de TF-IDF waarde van het sleutelwoord relatief tot de categorie. Deze waarden worden voor de sleutelwoorden opgeteld, zodat een totaalscore is bepaald per categorie. 
	
	\begin{equation}
		\text{Categorie}: max_j(\sum_{i=1}^5 p_{ij}) = max_j(\sum_{i=1}^5 \frac{occ_{ij}}{occ_{i\*}} \cdot \log{\frac{count(j)}{occ_{i\*}}})
	\end{equation}
	
	CultuurApp kijkt nu welke categorie het meest waarschijnlijk is en wijst deze categorie toe aan het monument.\\
	\\
	Door bij een monument waarvan de categorie bekend is de categorie 'te vergeten' en daarna de categorie te voorspellen, was het mogelijk om de juistheid van de voorspelling te controleren. Aan de hand van textuele analyse is het CultuurApp gelukt om 86\%, dus $\pm$ 19200 categorie\"en juist te voorspellen.
