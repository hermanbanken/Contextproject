...
\begin{enumerate}
	\reqitem{harvest-flickr} Er is een systeemcomponent dat live foto's kan ophalen van Flickr\footnote{http://www.flickr.com/services/api/} aan de hand van de geografische locatie, tags en zoektermen.
	\reqitem{detail-flickr} De website toont foto's uit de buurt van het monument van Flickr wanneer een toerist de detailpagina van een monument bezoekt. Deze foto's worden opgehaald bij het bezoeken van de detailpagina en alleen de url van de foto's worden opgeslagen.
\end{enumerate}
            
\rsubsection{Completeren dataset}
\begin{enumerate}
	\reqitem{analyse-offline-images} Foto's kunnen offline worden geanalyseerd door middel van visuele analyse. Hiervoor wordt ImageJ\footnote{http://rsbweb.nih.gov/ij/} gebruikt. De resultaten van de analyse worden naar de server ge-upload. Zie ook \reqref{analyse-offline-fetch} en \reqref{analyse-offline-push}.
 	\reqitem{analyse-online-text} Tekst kan online worden geanalyseerd met Naive Bayesiaanse filters. Indien er een dienst wordt gevonden die dit kan doen (uClassify.com is een kandidaat) dan wordt deze gebruikt, zo niet dan wordt dit ge\"implementeerd.
	\reqitem{analyse-coupling} Aan de hand van de resultaten van de visuele en textuele analyse worden monumenten gekoppeld aan bestaande categorie\"en, namelijk die van andere monumenten uit de data-set. Zie ook \reqref{analyse-offline-categorize}.
	\reqitem{search-analyse} De toerist kan zoeken naar 'aanbevolen' (vergelijkbare) monumenten. De resultaten van de visuele en textuele analyse tellen mee in de sortering van de resultaten. Ook de locatie, titel en categorie en gebruikers-interactie-informatie speelt mee. Zie ook \reqref{analyse-online-use} en \reqref{overview-gps}.
\end{enumerate}
...
\begin{enumerate}
	...
	\reqitem{harvest-detail-weather} Op de detailpagina wordt de weersverwachting rond het monument getoond. Zie ook \reqref{detail-weather}.
\end{enumerate}
...
\begin{enumerate}
	...
	\reqitem{detail-places} Op de detailpagina worden faciliteiten getoond op een kaartje van de omgeving rondom het monument.
	\reqitem{overview-places}  Op de overzichtspagina worden faciliteiten getoond. De toerist kan aangeven welke categorie\"en faciliteiten getoond moeten worden op het kaartje.
\end{enumerate}
                
\rsubsection{Monumenten vergelijken aan de hand van foto's}
\begin{enumerate}
		\reqitem{analyse-offline-fetch} Er is een lokaal systeem dat nog niet geanalyseerde foto's kan ophalen van de webserver en deze kan analyseren met behulp van de Java-tool ImageJ\footnote{http://rsbweb.nih.gov/ij/}.
		\reqitem{analyse-offline-push} Het lokale systeem kan de resultaten van de analyse uploaden naar de webserver.
		\reqitem{analyse-online-use} De webserver kan zoeken in de resultaten van de analyse om bij \'e\'en afbeelding andere afbeeldingen te vinden die gelijkend zijn.
		\reqitem{analyse-offline-visualize} Het lokale systeem kan de resultaten van de analyse visueel uitzetten met behulp van de Java-tool  ImagePlot\footnote{http://lab.softwarestudies.com/p/imageplot.html}.
		\reqitem{analyse-offline-categorize} Vanuit het lokale systeem zijn nog niet gecategoriseerde monumenten snel handmatig in te delen in categorie\"en door ze te groeperen aan de hand van de resultaten van de visuele analyse.
		\reqitem{detail-analyse} De toerist kan onderaan de detailpagina visueel vergelijkbare monumenten bekijken.
\end{enumerate}
...
\end{enumerate}
	...
	\begin{enumerate}
		\reqitem{overview-maps} Het systeem moet de gefilterde monumenten als spelden weergeven op een Google Maps\footnote{\label{googlemaps} http://code.google.com/apis/maps/index.html} kaart.	
	...
	\begin{enumerate}
		...
		\reqitem{detail-flickr} De detailpagina toont Flickrfoto's uit de buurt van het monument.
		\reqitem{detail-4sq} \label{views-checkins} De detailpagina toont het recente aantal checkins op Foursquare (zie ook \ref{4sq-checkins}).
		\reqitem{detail-weather} De detailpagina toont de weersverwachting rond het monument.
		\reqitem{detail-places} De detailpagina toont faciliteiten in de buurt van het monument in een kaartje.
		\reqitem{detail-analyse} De detailpagina toont visueel gelijkende monumenten.
		\reqitem{detail-favorize} De detailpagina bevat een knop waarmee een CultuurApp-gebruiker het monument aan zijn favorietenlijst kan toevoegen.
	\end{enumerate}
...
\begin{enumerate}
	\reqitem{detail-favorize} Een CultuurApp-gebruiker kan monumenten toevoegen aan zijn favorietenlijst.
